...
namespace stFacturacion.Controllers
{
    public class TallasController : Controller
    {
        ...
        public ActionResult Create()
        {
            ...
            return PartialView("_CreateTalla");
        }

        [HttpPost]
        public ActionResult Create(Talla talla)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var dbTalla = db.Tallas.Where(g => g.descripciontalla == talla.descripciontalla).SingleOrDefault();
                    return Json(new { Talla = dbTalla, Error = string.Empty });
                }
                else
                {
